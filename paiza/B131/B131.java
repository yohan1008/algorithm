 ...
 * 現在地と目的地さえ分かれば運賃表で移動にかかる運賃を検索することができる。そして検索した運賃を順に加算していき、合計運賃を計算する。
 * 同じ番号の駅に移動する場合は特に処理が不要なため、スキップさせる。
 * <p>
 * 運賃表はＮ個の各路線は同じ駅数をもっているため、2次元配列に保持することとし、
 * 各駅を移動するルートも同様[路線、目的地]の形で固定で入力されるため、2次元配列で保持をすることにする。
 *
 * @author kang yohan
 */
...
public class B131 {
    ...
    static void calcTotalFare(int routeNo, int destinationStation) {
        ...
        //マイナスの場合はプラスに変換する
        int fare =  Math.abs(toCurStationFare - toDesStationFare);
