 ...
 * 何回繰り返せば良いか？が予め分からないことから、全員が乗り込むまではwhile文で繰り返す形としました。
 * 
...
public class B108Review2 {
	 ...
	 *ゴンドラの合計乗車人数を求める。
	 *
	 * @param poepleNum 乗車人数
	 */
	private static void calcTotalPassengers(int peopleNum) {
		// グループに一人以上いる場合はゴンドラに乗せる。
		// 直前の処理で既にグループ全員がゴンドラに乗っている（peopleNumが0以下）の場合はこのグループの処理を終えて次のグループを処理する。
		while (peopleNum >= 1) {

			// 乗車するゴンドラの定員を取得する
			int capacity = gondolaCapacity[gondolaIdx];

			// 乗車人数がゴンドラの定員以内であれば今回のグループは全員が乗車したためループ処理を終了して次のグループを処理する。
			if (peopleNum <= capacity) {
				gondolaTotalPassengers[gondolaIdx] += peopleNum;
			} else {
				gondolaTotalPassengers[gondolaIdx] += capacity;
			}

			// ゴンドラに乗った人数をグループから取り除く。
			peopleNum -= capacity;
